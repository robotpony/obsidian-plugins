...
Each row shows branch and git status, item counts, and a relative "recently updated" date (the repo's `CHANGELOG.md`, falling back to `README.md`, whichever it has). The sort icon beside the filter box switches between Default, Name (A–Z), Most items, Needs attention (dirty git status or an open bug), and Recently updated.
