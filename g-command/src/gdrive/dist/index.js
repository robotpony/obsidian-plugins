...
const server = new Server({ name: "g-command/gdrive", version: "1.0.0" }, { capabilities: { resources: {}, tools: {} } });
server.setRequestHandler(ListResourcesRequestSchema, async (request) => {
    // cursor holds a subfolder path for pagination-by-folder
    const folder = request.params?.cursor ?? "";
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: "search",
            description: "Search for files in Google Drive by filename. Returns matching file paths and types. " +
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    const term = request.params.arguments?.query;
    if (!term) {
...
async function checkRclone() {
    ...
    try {
        ...
        console.error("Error: rclone not found in PATH.\n" +
    ...
    try {
        ...
        console.error(`Error: could not connect to rclone remote "${REMOTE}".\n` +
            `Run:  rclone config\n` +
